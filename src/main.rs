...
async fn main() -> std::io::Result<()> {
    ...
    let heartbeat_rate_limiter: web::Data<RateLimiter<IpAddr, DefaultKeyedStateStore<IpAddr>, DefaultClock>> = 
        web::Data::new(RateLimiter::keyed(config.heartbeat_quota()));
    
    let server_list_rate_limiter: web::Data<RateLimiter<IpAddr, DefaultKeyedStateStore<IpAddr>, DefaultClock>> = 
        web::Data::new(RateLimiter::keyed(config.server_list_quota()));
    
    let server_delete_rate_limiter: web::Data<RateLimiter<IpAddr, DefaultKeyedStateStore<IpAddr>, DefaultClock>> = 
